...
var Stringify = function(options) {
  ...
  options.commaNoLine = typeof options.commaNoLine === 'undefined' ? false : true;
  ...
  self._value = function (value, opt) {
    ...
    if (res.indexOf(',')>=0) {
      var err = new Error('values in csv format can\'t contain commas, value: \''+res+'\'');
      self.emit('error', err);
      if (o.commaNoLine) {
        throw err;
      }
...
var Parse = function(options) {
  ...
  self.parse = function (text, columns, opt) {
    ...
    function isExtraRow() {
      // Exclude extra rows
      if (opt.hasOwnProperty('extraRows')) {
        for( var len=opt.extraRows.length, i=len-1; i>=0; --i) {
          var extraRow = opt.extraRows[i];
