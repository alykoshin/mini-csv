...
var Stringify = function(options) {
  ...
  self._value = function (value, opt) {
    ...
    if (res.indexOf( o.separator )>=0) {
      var err = new Error('values in csv format can\'t contain separator characters, value: \''+res+'\'');
      self.emit('error', err);
      if (o.commaNoLine) {
        throw err;
      }
...
var Parse = function(options) {
  ...
  options.separator  = options.separator || ',';
  options.comment    = options.coment || '';
  ...
  self._line = function (string, opt) {
    ...
    for (var len = string.length, i = 0; i < len; ++i) {
      ...
      console.log('}}',i,c,o.separator)

      if (c === o.quote) {
        inQuotes = !inQuotes;

      } else if (c === o.separator && !inQuotes) {
  ...
  self._splitLines = function(text) {
    const res = (typeof text==='string' && text.length>0) ? text.split(/\r?\n/g) : [];
    console.log('>>>', res[0], '>>', res[2], '>', res[3])
    return res;
    //return text.split(/\r?\n/);
  };
  ...
  self.parse = function (text, columns, opt) {
    ...
    for (var linesLen=lines.length, linesIdx=0; linesIdx<linesLen; ++linesIdx) {
      ...
      if (o.comment!=='' && line.substring(0,o.comment.length) === o.comment) { // line starts with comment character

      } else if (columns && columns.length > 0) {      // parse this as regular line with column definitions
